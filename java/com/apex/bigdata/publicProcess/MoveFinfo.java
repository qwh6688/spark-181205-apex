package com.apex.bigdata.publicProcess;
...
/**
 * Created by 守门员 on 2018/12/7.
 */
public class MoveFinfo {
    private static Logger logger = LoggerFactory.getLogger(MoveFinfo.class);
    ...
    private static void moveData(Map<String, Object> job) {
        ...
        if (Integer.parseInt(job.get("sync_type").toString()) == 2) {
            ...
            SparkRuntime.initSparkResultTempView("spark" + table_name, DB, "info_" + table_name);
            System.out.println(getOverwriteSql(job, fileds, "spark" + table_name, 0).toString());
            SparkRuntime.exec(getOverwriteSql(job, fileds, "spark" + table_name, 0).toString());
